...
if button_1:
    ...
    home_image = Image.open('utilities/homepage.jpg')
    st.image(home_image)
    st.title('Welcome to our Resume Analyzer')
    st.subheader('Features Available')
    st.markdown("- Performance Metrics: Comprehensive quality and relevance evaluation of entire resumes.")
    st.markdown("- Anomaly Detection: Pinpoint inconsistencies, backed by an F1 score visualization. ")
    st.markdown("- Covariate Shift: Witness the evolution of data over time through intuitive bar graphs.")
    st.markdown("- Univariate Shift: Keep an eye on trending resume keywords.")
else:
    image = Image.open('utilities/system_health.png')
    st.image(image, caption='System health')
